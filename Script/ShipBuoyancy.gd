...
func _ready() -> void:
	# Inicializa o ângulo alvo com a rotação atual do barco para não dar tranco no início
...
func _physics_process(delta: float) -> void:
	...
	# Interpola suavemente a rotação atual para o ângulo alvo
	...
	# --- CÁLCULO DE FLUTUAÇÃO ---
	var center := global_position
	var bow_pos   := center + (transform.basis.z * probe_bow.z)
	var stern_pos := center + (transform.basis.z * probe_stern.z)
	var port_pos  := center + (transform.basis.x * probe_port.x)
	var stbd_pos  := center + (transform.basis.x * probe_stbd.x)
...
func comando_acelerar():
	velocidade_frente = VEL_MAX

func comando_parar():
	velocidade_frente = 0.0

# adiciona ou subtrai 90 graus (PI / 2 radianos)
func comando_virar_fixo(direcao: int):
	#  1 = esquerda,-1 = direita
	angulo_alvo_y += direcao * (PI / 2.0)
